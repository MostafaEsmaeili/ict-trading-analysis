...
19. **MSS-vs-SwingPointDetector ordering (WP3 FSM sequencing — adversarial-verify finding, PR #4 review).** `MarketStructureShiftDetector.FindBrokenSwing` correctly skips `!IsActive` swings, but `SwingPointDetector.BreachClosedSwings` (`InvalidateOnCloseBeyond` default true) Breaches the very swing the displacement candle closes beyond. If the confluence pipeline runs SwingPointDetector **before** MSS on the same displacement candle, the swing is pre-Breached and the legitimate MSS is dropped (latent in WP1 — no pipeline yet; the unit test exercises MSS in isolation). WP3 must pin one of: (a) MSS evaluated before BreachClosedSwings on a candle; (b) BreachClosedSwings only breaches on the candle **following** the close-beyond; (c) `FindBrokenSwing` accepts a swing Breached by **this same** candle (match Breach time/level) while still excluding earlier-bar Breached/Consumed swings. Add an integration test running SwingPointDetector → MSS on one displacement candle asserting the MSS still fires.
