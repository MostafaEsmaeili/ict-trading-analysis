...
Enum (closed; bind weights/required by member name; `ValidateOnStart` rejects unknown names). Grading (TGR-4): `score = Σ(matched weights)/Σ(applicable weights) ×100`; **A ≥80 & all required**, else **B if all required** (the bare-required 63 is no longer suppressed to C), **Reject** if any required missing. The B/C numeric thresholds are display-band labels; alert floor lets A/B through.
...
20. **Confluence grading denominator / alert floor — RESOLVED (issue #49, PR #50 / decisions register TGR-4).** `SetupCandidate` scores against `applicable = the constant set of all weighted conditions` (Σ default weights = 9.75). A consequence of the §2.5.3 default weights: a setup matching ALL 8 RequiredConditions but zero optional confluences scores Σ(required weighted)=6.15 ⇒ **63 ⇒ Grade C**, i.e. **below the 65 alert floor** — so "all required true" is necessary but not sufficient for a B/tradeable alert; some optional confluence (OTE/OB/SMT/macro) is needed to clear 65. This is internally consistent with §2.5.4 as written (B = all-required AND ≥65) and is unreachable on today's pipeline (no `KillzoneEntry` / `DrawTargetRrMet` emitter yet), but it is a genuine model/UX decision: confirm with the transcripts/`ict-domain-expert` whether all-required should auto-clear B (e.g. weight retune, a lower floor, or `applicable` = required∪matched) **before** the alerting/priced-Setup WP, and add a test pinning the bare-required score. §2.5.4. **RESOLUTION (TGR-4):** an all-RequiredConditions setup auto-clears to Grade **B** (A only at the A threshold); the 0–100 score is the within-grade sorter, not a floor — `SetupScorer.GradeFor` no longer returns C for an all-required setup. The bare-required **63** is pinned by a regression test; Σ(applicable) stays 9.75 (EG-2). A is unreachable (~77) until the optional emitters ship. See `docs/ict-core-model-decisions.md`.
...
29. **ExitManager deferrals (issue #28, PR #29 — §2.5.9 Slice C cut 2a: the orchestrator, no time-exit).** Cut 2a ships the pure `ExitManager.Decide(PaperTrade, Candle, ExitContext) → ExitPlan` (DECIDE-only): the §3.4 **protective-fill → scale → trail** precedence (fill closes the whole `RemainingSize` and short-circuits; else the surviving bar takes the **once-only** T1 partial sized `PartialFraction × the ORIGINAL Size` at the partial LEVEL via `ComputeExitLeg`, then ratchets via `StopTrailPolicy`); an **apply-ordered** `ExitPlan`/`ExitAction` (scale before move-stop, both stamped at the `ExitContext.BarCloseUtc` so the aggregate timeline stays monotonic); composes the injected `IFillEvaluator`/`IStopTrailPolicy`/`IExecutionCostModel`. Adversarially verified (**505k Decide-then-Apply cycles, 0 throws**). Deferred to **cut 2b+**: **(a) the max-hold / no-overnight time-exit** — reads `TimeframePolicy.MaxHold` (already on the policy) for `barCloseUtc − OpenedAtUtc ≥ MaxHold`, and a `NoOvernightBoundary { NyMidnight (default), NyFxClose1700 (deferred) }` decided via `NyClock` over the entry-vs-bar NY dates; it inserts as **protective-fill → time-exit → scale → trail** (overrides a same-bar scale/trail but NEVER a real fill), closing at `candle.Close` with `TradeCloseReason.TimeExit`; `ExitContext` is the forward-compat seam (carries `BarCloseUtc` today). **(b)** lot-step **flooring** of the partial leg (`PartialFraction × Size` is booked raw today — apply-safe only because `PartialFraction ∈ (0,1)` is `Validate()`+`ValidateOnStart`-enforced; floor it with the multi-partial/SD-ladder work). **(c)** the §2.5.8 `level+spread` fill-price worsening (stays in `FillEvaluator`); slippage/swap; the post-confirmation Armed/Triggered (Pending→Open) entry-arming that opens the trade the orchestrator then manages; the Host `Ict:Execution:Management*` binding + `ValidateOnStart` (with the `FillSimulation` host that calls `Decide` and applies the plan).

30. **Time-exit deferrals (issue #31, PR #32 — §2.5.9 Slice C cut 2b: the max-hold / no-overnight time-exit).** Cut 2b ships item 29(a): the time-exit woven into `ExitManager.Decide` as the second precedence rung (**protective-fill → time-exit → scale → trail**). `TimeExitFires` = **max-hold** (`BarClose − Open ≥ StyleSettings.MaxHoldMinutes`, pure UTC, inclusive; Intraday 120 — Mentorship-verbatim Ep21, read from the existing per-style config, NO new literal) **OR** **no-overnight** (`NyClock.NewYorkDate(open) != NyClock.NewYorkDate(barClose)`, the only DST-aware NY-date path, §2.1 00:00-NY boundary), gated on `StyleSettings.AllowOvernight == false`; both → ONE whole-`RemainingSize` `Close` at `candle.Close` as `TimeExit`, costed via `ComputeExitLeg`, R still vs the frozen 1R. A real stop/runner fill on the same bar ALWAYS outranks the time-exit (books the level, never a flattering close); the time-exit overrides a same-bar scale/trail. `NoOvernightBoundary { NyMidnight (default), NyFxClose1700 (deferred) }` on `ExitManagementOptions`; the FX-close boundary is **double-blocked** (rejected by `Validate()` + throws `NotSupportedException` if reached). `ExitManager` ctor now injects `NyClock` + `TradeStyleOptions`. Adversarially verified (~275k cases × 4 styles × both 2024 DST transitions × 5 process zones, ZERO invariant violations; 12/12 ICT checklist; guardrail 7/7). Deferred: **(a)** `NyFxClose1700` (17:00 ET FX rollover — couples to the swap/rollover cost math) + an explicit **Friday-NY-close / weekend-gap** boundary for the `AllowOvernight=true` styles (Swing/Position). **(b)** **sub-bar / tick time-exit precision** — max-hold is evaluated at BAR CLOSE so the realized flatten honors the cap to within one bar at the entry timeframe (faithful to Ep21's discretionary "two hours maximum"); revisit when the §5.2 tick-replay path lands. **(c)** the Swing 14400 / Position 43200 `MaxHoldMinutes` stay INVENTED (transcripts say only "days/weeks"); only Intraday 120 + the no-overnight rule are Mentorship-verbatim. **(d)** the Host `Ict:Execution:Management` (`NoOvernightBoundary`) + `Ict:TradeStyles` binding + `ValidateOnStart` and the `IExitManager` DI registration (must inject the shared `TimeProvider.System`-backed `NyClock` singleton) land with the consuming `FillSimulation` host that calls `Decide` and applies the plan. **Next WP5 slice:** the post-confirmation **Armed/Triggered** (Pending→Open) entry-arming + the slippage/session-stepped-spread/swap cost follow-ons (swap is now self-policing — the no-overnight time-exit guarantees 0 nights for Intraday/Scalp) + lot-step flooring of the partial leg.

31. **EntryFillEvaluator deferrals (issue #33, PR #35 — Armed/Triggered entry-arming cut 1: the pure limit-touch DECISION).** Cut 1 ships the pure `IEntryFillEvaluator`/`EntryFillEvaluator.Evaluate(Setup, Candle) → EntryFillDecision` (§2.5.1 step 7; Mentorship FULL PLAYLIST:876-877/:933/:2817/:3097): ICT enters on a resting LIMIT at the OTE/FVG level — long fills `candle.Low ≤ plan.Entry` (the **discount-side** retrace — looks like a long STOP operator but is the limit, the load-bearing fidelity check), short `candle.High ≥ plan.Entry` (premium), bar High/Low touch, inclusive (§2.5.8), filled at the LIMIT LEVEL (planned 1R == booked 1R), spread left to `ComputeEntryLeg` (no double-count), pure/clock-free. `EntryFillDecision { Hold, Filled }` mirrors `FillDecision`; no aggregate/account changes. Adversarially verified (310k cases, 7 invariants HOLD, negative-control-validated). Deferred to **cut 2 (the orchestrator / armed-lifecycle)**: **(a) the same-bar entry-then-stop −1R straddle** — a `Filled` decision can be returned for a bar whose Low/High ALSO breached the stop; the orchestrator MUST resolve it worst-case (StopFirst, consistent with `FillEvaluator`) on the fill bar BEFORE the next-bar exit pass, or it books a phantom favorable outcome (a future `EntryFillDecision` may carry a `stop-also-touched-this-bar` flag so the orchestrator has the signal without re-reading the bar). **(b)** the `ArmedEntry` aggregate (`ArmedEntryStatus { Armed, Triggered, Cancelled, Expired }`, pure/clock-free; an `EntryContext` mirrors `ExitContext`) + the **no-chase cancellation** precedence: setup-invalidation/stop-through > killzone-end > no-overnight (NY-midnight) > assembly-ageout (`MaxAssemblyBars`) > max-wait (INVENTED, provenance-flagged — no transcript max-WAIT for a pending limit). **(c)** **risk reservation at ARM time** (a resting limit is committed exposure under the ≈5% cap, §2.5.10) — recommend an id-keyed `PaperAccount.Reserve(id, budget)`/`Release(id)` carried over to the produced trade id on trigger; `TradeStatus`/`TradeLifecycle`/`PaperAccount.Settle` stay byte-unchanged (armed is a SEPARATE aggregate, not a Pending `TradeStatus`). **(d)** `EntryMode { Armed (faithful default), Immediate }` (`Ict:Execution:Entry:Mode`) — keep the §5.1 immediate-open path for the existing already-open-`PaperTrade` tests + as a what-if escape hatch. **(e)** the orchestrator that APPLIES the entry fill (calls `PaperTradeFactory.Open` stamped at the fill bar-close). The `ict-domain-expert` spec flagged **2 contested sub-decisions** worth a focused **design judge-panel**: (1) risk-reservation wiring — id-keyed `Reserve` on `PaperAccount` (A, recommended) vs reserve-at-trigger (B, breaches the cap on a same-bar fill burst) vs a separate `ArmedBook` aggregate (C); (2) the same-bar straddle boundary — entry evaluator emits trigger-only + orchestrator re-feeds the exit chain (A, recommended, single source of truth) vs a fused fill-then-stop −1R in the entry evaluator (B, drift risk) vs an `EntryPlan` precedence object mirroring `ExitPlan` (C).

32. **Armed-entry reservation deferrals (issue #36, PR #37 — entry-arming cut 2a: the arm-time reservation MECHANISM; design judge-panel `wf_56100ce0-872`, Design 1 unanimous 9/9/9).** Cut 2a ships `PaperAccount.Reserve(Guid, Money)` (id-keyed, extends the EXISTING `_reservedRiskByTrade` ledger through the same `CanOpen`/`OpenRisk` gate → ONE cap owner; a resting limit is committed exposure §2.5.10), the `ArmedEntry` resting aggregate (`ArmedEntryStatus { Armed, Triggered }`, id == the future trade id, raises `EntryArmed`/`EntryTriggered`), and `PaperTradeFactory.Arm` (sizes once + reserves, **atomic** — builds the entity before reserving) + `OpenArmed` (opens under the reservation's Guid WITHOUT `RegisterOpen` — the trigger is a **key re-label**, so `Settle` is byte-unchanged; the reserved `Money` == the trade's derived `RiskBudget` by construction, `PriceToPips ≡ /PipSize`). Adversarially verified ~92k cases (6 invariants HOLD, 0 drift, negative-control-validated; reviewer Should-fix [Arm atomicity] + Notes [OpenArmed fail-fast, domain events] all hardened). **LOCKED panel decisions for cut 2b:** Reservation **A** (id-keyed reserve carried to the trade id, NOT reserve-at-trigger [cap-breach window] nor a separate `ArmedBook` [split invariant]); Straddle **A** (the orchestrator opens at the bar-close then RE-FEEDS the same candle to the exit `FillEvaluator` — the ONE −1R worst-case authority — so the entry path can't book a phantom win; the equal-timestamp same-bar open→close is legal because `PaperTrade.GuardActivityTime` is non-strict and the ctor seeds `_lastActivityAtUtc = openedAtUtc`, so the orchestrator MUST open at the bar-close time — the load-bearing invariant + its mandatory test). Deferred to **cut 2b (the `EntryManager` orchestrator)**: **(a)** `PaperAccount.Release(Guid)` — symmetric removal of the same ledger key; REQUIRED because a stale arm that never triggers/settles leaks its risk against the cap indefinitely until cancellation releases it (flagged by the verifier). **(b)** the per-candle no-chase cancellation PRECEDENCE (setup-invalidation/stop-through > killzone-end > no-overnight(NY-midnight) > assembly-ageout(`MaxAssemblyBars`) > max-wait[INVENTED, provenance-flagged]) — each `Cancel(reason)` Releases the reservation; add `ArmedEntryStatus.Cancelled`. **(c)** the pure `IEntryManager.Decide(ArmedEntry, Candle, EntryContext) → EntryPlan` (mirroring `ExitManager`→`ExitPlan`/`ExitAction`/`ExitContext`), apply-ordered actions {Open, Close (the same-bar straddle −1R), Cancel}, pre-computing `ComputeEntryLeg`/`ComputeExitLeg` costs. **(d)** `EntryMode { Armed (default), Immediate }` (`Ict:Execution:Entry`, `EntryManagementOptions` POCO with the `MaxAssemblyBars`/INVENTED-`MaxWait`) — keep the §5.1 immediate-open path for the existing tests. **(e)** the steady-state post-fill exit hand-off (the opened trade then driven by the existing `ExitManager` per candle — wired by the module orchestrator, a later slice); persistence of the resting `ArmedEntry` (WP2); the Host `Ict:Execution:Entry` binding + `ValidateOnStart`.

33. **EntryManager fill + straddle deferrals (issue #39, PR #40 — entry-arming cut 2b-i).** Cut 2b-i ships the pure `IEntryManager.Decide(ArmedEntry, Candle, EntryContext) → EntryPlan` (the §2.5.1-step-7 fill path, mirroring `ExitManager`): a no-touch bar is `NoOp`; a fill emits `Open` at the bar-close; the same-bar entry-then-stop straddle is resolved by re-feeding the SAME candle to the exit `FillEvaluator` (the ONE StopFirst authority) on a would-be transient trade (events cleared) → `[Open, Close]` = −1R; a same-bar runner is NOT credited (only `Open`). `ArmedEntry` carries `PipSize`/`ValuePerPip`; `OpenArmed` reads them (dropped the spec params — removes a drift class). `EntryContext`/`EntryAction`/`EntryPlan` mirror the exit VOs. Adversarially verified (640k cases, 7 invariants HOLD, negative-control-validated; ict-domain-expert CONFORMANT, guardrail 7/7, pr-reviewer APPROVE). Deferred to **cut 2b-ii**: **(a)** the **no-chase cancellation** precedence — killzone-end (`KillzoneClassification.IsActiveEntryFor`) > no-overnight (`NyClock.NewYorkDate(arm) != NewYorkDate(bar)`) > max-wait (INVENTED, provenance-flagged); the structural setup-invalidation cancel needs `MarketContext`/MSS state (further deferred), and bar-count assembly-ageout needs a bar counter the stateless `Decide` lacks (use a time-based proxy or defer). Each cancel `→ Cancel(reason)` + `PaperAccount.Release(Guid)` (symmetric ledger removal) + `ArmedEntry.Cancel`/`ArmedEntryStatus.Cancelled` + `EntryCancelReason` enum + `EntryCancelled` event; needed so a stale arm doesn't leak the cap. **(b)** `EntryMode { Armed (default), Immediate }` (`Ict:Execution:Entry`, `EntryManagementOptions`) — selects the orchestrator vs the §5.1 immediate-open path; the module orchestrator that branches on it. **(c) The entry→exit same-bar re-feed obligation (reviewer-flagged):** the module orchestrator wiring entry→exit MUST re-feed the SAME bar to the exit pass after `OpenArmed`, else a same-bar RUNNER that genuinely traded on the entry bar is missed (the entry path conservatively defers it); and it must promptly settle a straddle-closed trade so the cap isn't transiently over-counted (the straddle leaves a closed-but-reserved trade until `Settle`). **(d)** the entry-fill `level+spread` worsening + slippage stay the deferred §5.4 cost lines (the straddle books only the round-trip spread/commission, gross of slippage — items 24(a)/25(b)/(c)).

34. **EntryManager no-chase cancellation deferrals (issue #41, PR #42 — entry-arming cut 2b-ii; completes the entry orchestrator).** Cut 2b-ii adds the no-chase cancellation precedence to `EntryManager.Decide`, run BEFORE the fill: **killzone-end** (`!KillzoneClock.IsActiveEntry(...)` — the bar left the active killzone, reusing the §4.6 hunt-set) **> max-wait** (INVENTED/provenance-flagged backstop, default 240). Each `→ EntryAction.Cancel` applied as `ArmedEntry.Cancel` + `PaperAccount.Release` (symmetric ledger removal — the cap self-heals). No-overnight is NOT a separate rung (no FX active killzone spans 00:00 NY → a midnight cross already trips killzone-end; load-bearing only for the exit time-exit's HELD trade). New: `EntryCancelReason`, `EntryAction.Cancel`, `ArmedEntry.Cancel`/`Cancelled`/`InstrumentClass`, `PaperAccount.Release`, `EntryCancelled` event, `EntryManagementOptions`/`EntryMode`. ict-domain-expert CONFORMANT + guardrail 7/7 + pr-reviewer APPROVE (no Critical/Should-fix). **(a) killzone classification AXIS — RESOLVED (review pass, PR #42):** cancellation now classifies the killzone from `candle.OpenTimeUtc`, the SAME axis `MarketContext`/`KillzoneEntryDetector` use, so a boundary-straddling bar is treated identically everywhere (max-wait still measured to the bar close). **Tracked follow-ups (both reviewers non-blocking):** **(b)** the no-overnight-subsumption rests on a config invariant (no selectable killzone spans 00:00 NY) NOT enforced in code — add a structural assertion in `KillzoneSchedule` (or re-introduce a cheap `NyClock.NewYorkDate(arm)!=NewYorkDate(bar)` backstop) so a future midnight-spanning killzone config can't let a limit survive overnight (max-wait 240 only bounds it to ~4h today). **(c)** an index-class cancellation test (the §2.5.7 split: an index arm past 10:40/11:00) to lock the instrument-class path. **(d)** Cancel+Release atomicity at the module applier (two aggregate calls — the factory-built arm can't mismatch, but the wiring must commit both together). **(e)** `EntryManagementOptions.Validate` upper sanity bound on `MaxWaitMinutes`. **(f)** the **entry→exit same-bar re-feed** (the module orchestrator must re-feed the same bar to the exit pass after `OpenArmed` so a same-bar runner isn't missed) + the Host `Ict:Execution:Entry` binding + `ValidateOnStart` + `IEntryManager` DI (with the consuming `FillSimulation`/module-orchestrator slice that selects `EntryMode` and runs `EntryManager`→`OpenArmed`→`ExitManager` per candle).
