...
internal sealed class ArmedEntryConfiguration : IEntityTypeConfiguration<ArmedEntry>
{
    public void Configure(EntityTypeBuilder<ArmedEntry> builder)
    {
        ...
        // A queryable MIRROR of the value also inside the Setup JSONB snapshot (PaperTradeFactory.Arm always passes
        // setup.StackedFartherBound, AND the snapshot's SetupDto now serialises it too, so the two cannot diverge on
        // reload) — this column is the runtime read (EntryManager.ResolveCancellation), not an independent source of truth.
