...
public sealed class BacktestOptimizer
{
    ...
    private IReadOnlyList<BacktestRequest> BuildCombinations(OptimizeRequest request)
    {
        ...
        // null/empty models → the canonical Ict2022 only; a list sweeps the setup-model axis (plan §16 — the
        // leaderboard then names the winning MODEL per asset/timeframe/style, "suggest the best setup").
        var models = request.Models is { Count: > 0 }
            ? request.Models.Select(m => (string?)m).Distinct(StringComparer.OrdinalIgnoreCase).ToArray()
            : [null];

        var seen = new HashSet<(string, string, string, decimal, int?, string, string)>();
                            ...
                            foreach (var subset in requiredSubsets)
                            {
                                foreach (var model in models)
                                {
                                    var combo = new BacktestRequest(
                                        ...
                                        request.ToUtc, minRequired, subset, model);
                                    ...
                                    var key = (
                                        symbol, _engine.ResolveTimeframe(combo).ToString(), style, risk, minRequired,
                                        subsetKey, model ?? "Ict2022");
                                    if (seen.Add(key))
                                    {
                                        combos.Add(combo);
                                    }
